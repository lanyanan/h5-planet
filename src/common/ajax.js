...
import client from "./client.interactive";
...
function drawUrl(url){
	let newUrl = url;
    let deviceInfo = client.clientGetDeviceInfo() || '{"appVer": "1.3.5"}';
    let appVer = JSON.parse(deviceInfo).appVer.substring(0,3);
    ...
    if(getUrlParam(window.location.href, '_static')){
        ...
        if(config.mock){
            ...
            newUrl += '&appVer='+appVer+'&appkey=jz-yyxq&v=1.0&locale=zh_CN&format=json';
        }
...
function resetStatic(name, content){
    if(getUrlParam(window.location.href, '_restatic')){
        ...
        $.ajax({
         type: "post",
         data: params,
         url: config.restaticUrl + "&appkey=jz-yyxq&v=1.0&locale=zh_CN&format=json",
...
function getSign(url, args, excludes){
    let secret = "200.153.148.155.197.199.156.198.150.150.151.151.151.155.197.201.196.201.151.149.151.150.196.201.196.147.196.200.196.198.149.148";
    let s = '';
